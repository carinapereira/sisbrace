...
namespace DataAccess
{
    public class OrigensDA : BaseDA
    {
        ...
        public bool InserirDA(Origens instancia)
        {
            ...
            query.Append(@" insert into origens(codigo, descricao)");
        ...
        public bool EditarDA(Origens instancia)
        {
            ...
            query.Append(@" update origens set codigo=@codigo, descricao=@descricao");
        ...
        public bool ExcluirDA(Origens instancia)
        {
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM origens WHERE id = {0}", instancia.Id));    
