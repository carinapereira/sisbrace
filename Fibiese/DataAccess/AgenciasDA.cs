...
namespace DataAccess
{
    public class AgenciasDA : BaseDA
    {
        ...
        public bool InserirDA(Agencias age)
        {
            ...
            query.Append(@"insert into agencias(codigo, descricao, cep,bairroId, Endereco, complemento, cidadeId, Ranking, bancoId)");
            query.Append(@"values(@codigo, @descricao,@cep,@bairroid,@Endereco,@complemento,@cidadeid,@Ranking,@bancoid)");	
        ...
        public bool EditarDA(Agencias age)
        {
            ...
            query.Append(@"UPDATE agencias SET codigo=@codigo, descricao=@descricao, CEP=@CEP");
            query.Append(@",bairroId=@bairroId,Endereco=@Endereco, complemento=@complemento, cidadeId=@cidadeId");
            query.Append(@",Ranking=@Ranking, bancoId=@bancoId ");
        ...
        public bool ExcluirDA(Agencias age)
        {
            ...
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM agencias WHERE id = {0}", age.Id));               
