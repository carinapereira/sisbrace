...
namespace DataAccess
{
    public class TitulosDA : BaseDA
    {
        ...
        public bool InserirDA(Titulos tit)
        {
            ...
            try
            {
                for (int i = 0; i < tit.Parcela; i++)
                {
                    ...
                    paramsToSP[3] = new SqlParameter("@pessoaid", tit.Pessoaid == null ? (object)DBNull.Value : tit.Pessoaid);
                    paramsToSP[4] = new SqlParameter("@portadorid", tit.Portadorid == null ? (object)DBNull.Value : tit.Portadorid);
                    paramsToSP[5] = new SqlParameter("@dtvencimento", dt_vencimento);
                    paramsToSP[6] = new SqlParameter("@dtemissao", dt_emi);
                    paramsToSP[7] = new SqlParameter("@tipodocumentoid", tit.TipoDocumentoId == null ? (object)DBNull.Value : tit.TipoDocumentoId);
                    paramsToSP[8] = new SqlParameter("@tipo", tit.Tipo);
                    paramsToSP[9] = new SqlParameter("@dtPagamento", tit.DtPagamento == null ? (object)DBNull.Value : tit.DtPagamento);
                    paramsToSP[10] = new SqlParameter("@valorPago", tit.ValorPago == null ? (object)DBNull.Value : tit.ValorPago);
                    ...
                    query.Append(@" insert into titulos(numero,parcela,valor,pessoaid,portadorid,dtvencimento,dtemissao,tipodocumentoid,tipo,dtPagamento,valorPago,obs)");
                    query.Append(@" values(@numero,@parcela,@valor,@pessoaid,@portadorid,@dtvencimento,@dtemissao,@tipodocumentoid,@tipo,@dtPagamento,@valorPago,@obs)");
        ...
        public bool EditarDA(Titulos tit)
        {
            ...
            paramsToSP[4] = new SqlParameter("@pessoaid", tit.Pessoaid == null ? (object)DBNull.Value : tit.Pessoaid);
            paramsToSP[5] = new SqlParameter("@portadorid", tit.Portadorid == null ? (object)DBNull.Value : tit.Portadorid);
            paramsToSP[6] = new SqlParameter("@dtvencimento", tit.DataVencimento);
            paramsToSP[7] = new SqlParameter("@dtemissao", tit.DataEmissao);
            paramsToSP[8] = new SqlParameter("@tipodocumentoid", tit.TipoDocumentoId == null ? (object)DBNull.Value : tit.TipoDocumentoId);
            paramsToSP[9] = new SqlParameter("@tipo", tit.Tipo);
            paramsToSP[10] = new SqlParameter("@dtPagamento", tit.DtPagamento == null ? (object)DBNull.Value : tit.DtPagamento);
            paramsToSP[11] = new SqlParameter("@valorPago", tit.ValorPago == null ? (object)DBNull.Value : tit.ValorPago);
            paramsToSP[12] = new SqlParameter("@obs", tit.Obs.ToUpper());
          	
            query.Append(@" update titulos set numero=@numero, parcela=@parcela, valor=@valor, pessoaId=@pessoaId, portadorId=@portadorId");
            query.Append(@" ,dtVencimento=@dtVencimento, dtEmissao=@dtEmissao, tipoDocumentoId=@tipoDocumentoId, tipo=@tipo ");
            query.Append(@" ,dtPagamento=@dtPagamento, valorPago=@valorPago, obs=@obs ");
        ...
        public bool ExcluirDA(Titulos tit)
        {
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM titulos WHERE id = {0}", tit.Id));
