...
namespace DataAccess
{
    public class MovimentosEstoqueDA
    {
        ...
        public bool InserirDA(MovimentosEstoque movEst)
        {
            ...
            query.Append(@" insert into movimentosestoque(vendaitensid, usuarioid, itemestoqueid, quantidade, notaentradaitensid, tipo, data)");
            query.Append(@" values(@vendaitensid, @usuarioid, @itemestoqueid, @quantidade, @notaentradaitensid, @tipo, @data)");
        ...
        public bool EditarDA(MovimentosEstoque movEst)
        {
            ...
            query.Append(@" update movimentosestoque set vendaItensid=@vendaItensid, usuarioid=@usuarioid,itemestoqueid=@itemestoqueid");
            query.Append(@" , quantidade=@quantidade, notaentradaitensid=@notaentradaitensid, tipo=@tipo, data=@data ");
        ...
        {            
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM movimentosestoque WHERE id = {0}", movEst.Id));    
