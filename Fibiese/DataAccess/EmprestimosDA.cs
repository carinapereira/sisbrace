...
namespace DataAccess
{
    public class EmprestimosDA : BaseDA
    {
        ...
        public Int32 InserirDA(Emprestimos instancia)
        {
            ...
            query.Append(@" insert into emprestimos(exemplarid, pessoaid)");
            query.Append(@" values(@exemplarid, @pessoaid)");
        ...
        public bool EditarDA(Emprestimos instancia)
        {
            ...
            query.Append(@" update emprestimos set exemplarid=@exemplarid, pessoaid=@pessoaid ");
        ...
        {           
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM emprestimos WHERE id = {0}", instancia.Id));
        ...
        public List<Emprestimos> PesquisarDA(string campo, string valor)
        {
            ...
            List<Emprestimos> emprestimos = CarregarObjEmprestimos(dr);
