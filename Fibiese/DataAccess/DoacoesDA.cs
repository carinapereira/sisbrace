...
namespace DataAccess
{
    public class DoacoesDA
    {
        ...
        public Int32 InserirDA(Doacoes doa)
        {
            ...
            query.Append(@" insert into doacoes(pessoaid, data, valor, usuarioid)");
            query.Append(@" values(@pessoaid, @data, @valor, @usuarioid)");
            query.Append(@" SELECT SCOPE_IDENTITY() ");
            try
            {
                var id = SqlHelper.ExecuteScalar(ConfigurationManager.ConnectionStrings["conexao"].ToString(),
        ...
        public bool EditarDA(Doacoes doa)
        {
            ...
            query.Append(@" update doacoes set pessoaid=@pessoaid, data=@data, valor=@valor, usuarioid=@usuarioid");
        ...
        {              
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM doacoes WHERE id = {0}", doa.Id));    
