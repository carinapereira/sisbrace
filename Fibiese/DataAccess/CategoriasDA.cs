...
namespace DataAccess
{
    public class CategoriasDA : BaseDA
    {
        ...
        public bool InserirDA(Categorias cat)
        {
            ...
            query.Append(@" insert into categorias(codigo, descricao)");
        ...
        public bool EditarDA(Categorias cat)
        {
            ...
            query.Append(@" update categorias set codigo=@codigo, descricao=@descricao");
        ...
        {          
            ...
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM permissoes WHERE categoriaid = {0}", cat.Id)); 

                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM categorias WHERE id = {0}", cat.Id));    
