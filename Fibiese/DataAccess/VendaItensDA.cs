...
namespace DataAccess
{
    public class VendaItensDA
    {
        ...
        public bool InserirDA(VendaItens venItEi, int usuarioID)
        {
            ...
            query.Append(@" insert into vendaitens(vendaId, quantidade, valor, desconto, situacao, itemEstoqueId) ");
            query.Append(@" values(@vendaId,@quantidade,@valor,@desconto,@situacao,@itemEstoqueId) ");	
            
            try
            {
                SqlHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings["conexao"].ToString(),
        ...
        public bool EditarDA(VendaItens venItEi)
        {
            ...
            query.Append(@" update vendaitens set vendaId=@vendaId, quantidade=@quantidade, valor=@valor, desconto=@desconto, situacao=@situacao, itemEstoqueId=@itemestoqueid");
        ...
        public bool ExcluirDA(VendaItens venItEi)
        {
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                           string.Format("DELETE FROM vendaitens WHERE id = {0}", venItEi.Id));    
