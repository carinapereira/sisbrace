...
namespace DataAccess
{
    public class PortadoresDA : BaseDA
    {
        ...
        public bool InserirDA(Portadores por)
        {
            ...
            paramsToSP[2] = new SqlParameter("@agenciaid", por.AgenciaId == null ? (object)DBNull.Value : por.AgenciaId);
            paramsToSP[3] = new SqlParameter("@bancoid", por.BancoId == null ? (object)DBNull.Value : por.BancoId);
            paramsToSP[4] = new SqlParameter("@contaid", por.ContaId == null ? (object)DBNull.Value : por.ContaId);
            paramsToSP[5] = new SqlParameter("@codCedente", por.CodCedente == null ? (object)DBNull.Value : por.CodCedente);
            paramsToSP[6] = new SqlParameter("@carteira", por.Carteira);

            query.Append(@" insert into portadores(codigo, descricao, agenciaid, bancoid, contaid, codcedente, carteira)");
            query.Append(@" values(@codigo, @descricao, @agenciaid, @bancoid, @contaid, @codcedente, @carteira)");
        ...
        public bool EditarDA(Portadores por)
        {
            ...
            paramsToSP[3] = new SqlParameter("@agenciaid", por.AgenciaId == null ? (object)DBNull.Value : por.AgenciaId);
            paramsToSP[4] = new SqlParameter("@bancoid", por.BancoId == null ? (object)DBNull.Value : por.BancoId);
            paramsToSP[5] = new SqlParameter("@contaid", por.ContaId == null ? (object)DBNull.Value : por.ContaId);
            paramsToSP[6] = new SqlParameter("@codCedente", por.CodCedente == null ? (object)DBNull.Value : por.CodCedente);
            paramsToSP[7] = new SqlParameter("@carteira", por.Carteira);

            query.Append(@" update portadores set codigo=@codigo, descricao=@descricao, agenciaid=@agenciaid, bancoid=@bancoid");
            query.Append(@" ,contaid=@contaid, codcedente=@codcedente, carteira=@carteira");
        ...
        public bool ExcluirDA(Portadores por)
        {
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM portadores WHERE id = {0}", por.Id));    
