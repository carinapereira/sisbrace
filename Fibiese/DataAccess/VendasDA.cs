...
namespace DataAccess
{
    public class VendasDA
    {
        ...
        public Int32 InserirDA(Vendas ven)
        {
            ...
            query.Append(@" insert into vendas(numero, pessoaid, usuarioid, data, situacao)");
            query.Append(@" values(@numero, @pessoaid, @usuarioid,@data,@situacao) SELECT SCOPE_IDENTITY() ");
        ...
        public bool EditarDA(Vendas ven)
        {
            ...
            query.Append(@" update vendas set numero=@numero, pessoaid=@pessoaid, usuarioid=@usuarioid, data=@data, situacao=@situacao ");
        ...
        public bool ExcluirDA(Vendas ven)
        {
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM vendas WHERE id = {0}", ven.Id));    
