...
namespace DataAccess
{
    public class ItensEstoqueDA : BaseDA
    {
        ...
        public bool InserirDA(ItensEstoque itEst)
        {
            ...
            query.Append(@" insert into itensestoque(status, controlaestoque, qtdminima, obraid, vlrcusto, vlrvenda, data)");
            query.Append(@" values(@status, @controlaestoque, @qtdminima, @obraid, @vlrcusto, @vlrvenda, @data)");
        ...
        public bool EditarDA(ItensEstoque itEst)
        {
            ...
            query.Append(@" update itensestoque set status=@status, controlaestoque=@controlaestoque, qtdminima=@qtdminima, obraid=@obraid");
            query.Append(@" , vlrcusto=@vlrcusto, vlrvenda=@vlrvenda, data=@data");
        ...
        {           
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM itensestoque WHERE id = {0}", itEst.Id));    
