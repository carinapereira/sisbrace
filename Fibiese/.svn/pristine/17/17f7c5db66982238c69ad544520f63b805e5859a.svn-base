...
namespace DataAccess
{
    public class CidadesDA : BaseDA
    {
        ...
        public bool InserirDA(Cidades cid)
        {
            ...
            paramsToSP[2] = new SqlParameter("@uf", cid.UF);

            query.Append(@"insert into cidades(codigo,descricao,uf) ");
            query.Append(@"values(@codigo,@descricao,@uf) ");
        ...
        public bool EditarDA(Cidades cid)
        {
            ...
            paramsToSP[3] = new SqlParameter("@uf", cid.UF);

            query.Append(@"update cidades set codigo=@codigo, descricao=@descricao, uf=@uf ");
        ...
        {           
            ...
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM cidades WHERE id = {0}", cid.Id));    
        ...
        public List<Cidades> PesquisaCidUfDA(string id_uf)
        {
            SqlDataReader dr = SqlHelper.ExecuteReader(ConfigurationManager.ConnectionStrings["conexao"].ToString(),
                                                              CommandType.Text, string.Format(@"SELECT * FROM CIDADES WHERE UF = '{0}'", id_uf));
