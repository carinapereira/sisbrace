...
namespace DataAccess
{
    public class ContasDA : BaseDA
    {
        ...
        public bool InserirDA(Contas con)
        {
            ...
            query.Append(@" insert into contas(codigo, descricao, digito, agenciaid, titular)");
            query.Append(@" values(@codigo, @descricao, @digito, @agenciaid, @titular)");
        ...
        public bool EditarDA(Contas con)
        {
            ...
            query.Append(@" update contas set codigo=@codigo, descricao=@descricao, digito=@digito, agenciaid=@agenciaid, titular=@titular");
        ...
        public bool ExcluirDA(Contas con)
        {
            try
            {
                SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["conexao"].ToString(), CommandType.Text,
                                          string.Format("DELETE FROM contas WHERE id = {0}", con.Id));    
